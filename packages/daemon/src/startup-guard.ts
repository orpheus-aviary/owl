...
import { type OwlConfig, isHexProfileId, normalizeServerUrl } from '@owl/core';
...
export function assertDaemonStartupSafe(config: OwlConfig, opts: StartupGuardOptions): void {
  ...
  // ⑥ shape validation (both modes) — run first so a typo'd field reports
  // clearly rather than mis-tripping a policy guard below.
  ...
  // ⑥ cloud URL / host field validation.
