...
export function getNearestAlarm(note: Note, now: Date): NearestAlarm | null {
  ...
  for (const tag of note.tags) {
    for (const candidate of alarmCandidates(tag, uniqueFreqs, now)) {
...
/**
 * Future-alarm candidates generated by a single tag: nothing for a non-/alarm
 * tag; the raw alarm time (if future) when the note has no frequency modifier;
 * otherwise one candidate per frequency (each advanced to its next occurrence).
 */
function alarmCandidates(tag: NoteTag, uniqueFreqs: Frequency[], now: Date): NearestAlarm[] {
  if (tag.tagType !== '/alarm' || !tag.tagValue) return [];
  const alarmTime = new Date(tag.tagValue);

  if (uniqueFreqs.length === 0) {
    // No frequency — only a candidate if the alarm is in the future.
    return alarmTime > now ? [{ time: alarmTime, tag, frequency: null }] : [];
  }

  const out: NearestAlarm[] = [];
  for (const freq of uniqueFreqs) {
    const next = getNextOccurrence(alarmTime, freq, now);
    if (next) out.push({ time: next, tag, frequency: freq });
  }
  return out;
}
