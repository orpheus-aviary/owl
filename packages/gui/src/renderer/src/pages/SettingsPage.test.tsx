 ...
 *   - default to 'shortcuts' when no/invalid `?tab=` is present
...
describe('SettingsPage tab deep-link', () => {
  ...
  it('falls back to 快捷键 when ?tab=bogus', () => {
    renderAt('/settings?tab=bogus');
    expect(screen.getByTestId('section-shortcuts')).toBeTruthy();
  });

  it('falls back to 快捷键 when no ?tab param', () => {
    renderAt('/settings');
    expect(screen.getByTestId('section-shortcuts')).toBeTruthy();
  });
});
