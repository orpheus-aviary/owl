...
## 当前阶段：**Stage 1 #1 owl-server 打包 ✅ + #2 A6 local CSRF ✅ + #3 重构一轮 ✅ 完成并 push origin/main（2026-07-16，13 commit `95c21bb`…`cac23dd`，全绿 + GUI 手测通过）**；下一步 = **Stage 1 #4 0.6 本地功能**（路线定稿 `docs/plans/2026-07-04-road-to-1.0.0.md`）。Phase A 核心 A0–A5 + Phase B 网页版 v1（B0–B4）+ Aω 1a 本地云 rig 均已完成并 push。
...
4. **⭐ 0.6 本地功能（下个会话起点）**（**跨 profile 视图已移出 → 1.0.0 后**）：W7 冲突合并（`@codemirror/merge`+counter 列，**独立稿**）· `resetAllStores` 免闪烁（替 `location.reload`）· web session UX（`/auth/session` 探针 +「记住我」opt-in）。
