...
**#5 移动端 web UI —— 全部完成并提 `main`**（设计 `docs/plans/2026-07-22-mobile-web-ui.md`：Steps 1–4 §15 · Step 5 修订导航模型 §16 · **Steps 6–9 + PWA §17**）：
- **Steps 1–5**（`20b7339`…`be95c32`）：useIsMobile+Sheet · 分壳+AppRoutes · 导航契约核心（SaveResult/loadNoteById/note-nav-guard/useOpenNote）· 移动导航壳 · **修订导航模型**（编辑器仅 `/note/:id` 详情、入口=浏览默认+文件整页、底栏 `[浏览,文件,提醒,待办]`+更多、删抽屉+加新建按钮）。
- **本会话 Steps 6–9 + PWA + 清理**：Step 6 单栏 Settings/Login/AI（`6afd80c`/`5254572`/`4c2bed6`）· Step 7 移动冲突 UI（`d2c571c`）· Step 8 ⭐文件页拖拽手柄（`a84522f`）· Step 9 浮动 TagBar（`cb944c9`）· Phase 2 PWA 元数据（`1b14bb6`）· 清理死 drawer 变体（`f3c219b`）。**每处移动分支 `useIsMobile` 门控，桌面零回归。**
- **⚠️ 留 Stage 2 真机验收**：Step 9 软键盘浮动 TagBar + DateTimePicker 视口校正（代码内 `REAL-DEVICE` 标注）· Phase 2 真 PWA 安装（HTTPS/A2HS）· §12 微调（maskable padding / iOS fixed 抖动）。**次要未做**：Browser 笔记行「常显 ⋯」（长按已可用，判可选）· A6 遗留 A4-off grace-quiesce + 可选守卫 `check-local-token-not-logged`（均不阻断）。

**前序**：#4 0.6 本地功能 ✅（①③④②，设计 `2026-07-16-0.6-local-features.md`）。**#1 owl-server ✅ + #2 A6 ✅ + #3 重构一轮 ✅**（push origin/main 2026-07-16）。Phase A 核心 A0–A5 + Phase B v1（B0–B4）+ Aω 1a 本地云 rig 均已完成并 push。**最终基线：core 542 / cli 139 / daemon 431 / gui 627 / e2e 0-fail / `just check` 9 守卫。**
...
**Stage 1（全本地、不碰真服务器，发版前先做完）—— ✅ 全部完成 2026-07-23：**
