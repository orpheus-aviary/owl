...
## 当前阶段：**Stage 1 #5 移动端兼容 web UI —— Step 5 ✅ 完成（导航模型已修订）**。设计 `docs/plans/2026-07-22-mobile-web-ui.md`（v7 + §15 实施记录 + **§16 修订导航模型**）。**Step 1–4 ✅**（`20b7339`…`966239e`，gui→566；**未 push**，与 #4 一起攒在本地，用户习惯攒着不 push）。**⭐ Step 5 ✅ 完成但未 push**（本地迭代，直接提 main 逐步小 commit；**用户 2026-07-23 拍板改导航模型**，见 §16）：

**Step 5 = 修订导航模型**（原「编辑 tab = master 笔记列表 + detail」作废）：编辑器**仅 `/note/:id` 详情**，入口页 = **浏览（默认，`/`→`/browser`）+ 文件（文件夹树整页 `/files`）**，底栏 `[浏览, 文件, 提醒, 待办]` + 更多。三块：**①编辑器引擎**（`resolveOpen` + `EditorPage` 拆壳 + `EditorPanel effectiveMode` + `NoteNavGuardDialog` 新 + `note-nav-guard` 3 选项[保存/放弃/打开笔记]+`requestLeave` + `MobileTopBar` 详情[返回走 guard/脏时保存高亮/保存-返回竞态]）· **②导航重构**（底栏 编辑→文件 + `FilesPage`[`FolderPanel variant=page`] + 删 `FolderDrawer`/☰ + `SyncStatusBar`→更多 + **新建笔记按钮**[顶栏 `/browser`·`/files`]）· **③入口迁移**（Browser/Reminders/Todo/Conflicts/NoteIdPill→`useOpenNote`；浏览手机单击开 + 禁拖 + 搜索/按钮两行；`useEditorShortcuts` opener 注入；`DeleteConfirmDialog` delete 按来源）。**多篇未保存安全**：提示只针对当前活动脏笔记，「打开笔记」跳这一篇无歧义。**桌面零回归**。gui 566→**601**。**下一步 = Step 6** 单栏 Settings/Login/AI → Step 7 移动冲突 UI → **Step 8 触摸打磨（含 FolderPanel 文件页独立拖拽手柄 = 用户要的「文件页按住拖动整理」）** → Step 9 浮动 TagBar → Phase 2（PWA + docs）。

**前序**：#4 0.6 本地功能 ✅ 全部完成（①③④②，设计 `2026-07-16-0.6-local-features.md`）。**#1 owl-server ✅ + #2 A6 ✅ + #3 重构一轮 ✅**（push origin/main 2026-07-16）。Phase A 核心 A0–A5 + Phase B v1（B0–B4）+ Aω 1a 本地云 rig 均已完成并 push。基线：core **542**/cli **139**/daemon **431**/gui **601**/e2e 29(0-fail)/`just check` 9 守卫。
