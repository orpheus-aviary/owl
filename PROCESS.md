...
## 当前阶段：**Stage 1 #4 0.6 本地功能实施中**（设计定稿 `docs/plans/2026-07-16-0.6-local-features.md` **v3.4「实施就绪」**，6 轮 review；顺序 ①→③→④→②）。**① 本地同步状态 ✅ + ③ 会话隔离原语 ✅（+ 手测新增「脏标签切换防护」）均完成 + GUI/真 rig 手测通过（2026-07-21）**，**下一步 = ④ web session UX**（之后 ②）。#1 owl-server ✅ + #2 A6 local CSRF ✅ + #3 重构一轮 ✅（三者已 push origin/main，2026-07-16，13 commit `95c21bb`…`cac23dd`）。Phase A 核心 A0–A5 + Phase B 网页版 v1（B0–B4）+ Aω 1a 本地云 rig 均已完成并 push。
...
4. **⭐ 0.6 本地功能实施中（设计 `docs/plans/2026-07-16-0.6-local-features.md` v3.4）**（**跨 profile 视图已移出 → 1.0.0 后**）；实施顺序 **①→③→④→②**：
   ...
   - **② W7 冲突手动解决/合并**（`resolveConflict` 抢占先于写 + core 独占 `.immediate()` 事务 + `@codemirror/merge` 模态；counter 列 D5 延后；触 core+daemon+**shared**+GUI）· **④ web session UX**（`probingToken` 复水 +「记住我」+ transport `onUnauthorized({usedToken})`；③ 已铺 shared `onDisconnect` 的 `usedToken`）。D1–D12 已定。**下一步 = ④**（③ 产出的 `activateSession`/`invalidateSession` 供 ④ 消费）。
