...
5. **移动端兼容 web UI**（**下一步**）：响应式 + 移动导航 + 触摸 + PWA（**不是 RN**；RN 是 1.0.0 后单独 app）。**独立设计稿。** 发网页版前做。
